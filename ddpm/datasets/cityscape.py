...
class Cityscapes_Pretraining_Dataset(VisionDataset):
    def __init__(self, cfg, root: Optional[str] = None, 
        ...
        super(Cityscapes_Pretraining_Dataset, self).__init__(self.root)
        ...
        if self.split == "all":
            for split in ["train", "test", "val"]:
                self.images_dir = os.path.join(self.root, "leftImg8bit_sequence", split)
            ...
            self.images_dir = os.path.join(self.root, "leftImg8bit_sequence", self.split)
            for city in os.listdir(self.images_dir):
                img_dir = os.path.join(self.images_dir, city)
                for file_name in os.listdir(img_dir):
                    self.images.append(os.path.join(img_dir, file_name))
...
class Cityscapes(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.transforms is not None:
            corrupted_image, _ = self.transforms(X, target)
            image, target = self.transforms(image, target)

        return image, corrupted_image, target
